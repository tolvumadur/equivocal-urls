...
try:
    ...
    res = sub.run([os.path.join(HERE, "wget"), desc], capture_output=True, input=bytes(url_b64, encoding="ascii")+b"\n")
    #res = sub.run(["valgrind", os.path.join(HERE, "wget-1.21", "src", "wget"), desc], capture_output=True, input=bytes(url_b64, encoding="ascii")+b"\n")

    if res.returncode != 0 or len(res.stderr) > 0:
        sys.stderr.write(str(res.stdout, encoding="utf-8", errors = "replace"))
        sys.stderr.write(f"Error ({res.returncode}) running wget parser: " + res.stderr.decode() + '\n')
        if res.returncode == -11:
            sys.stderr.write("Probably a segfault in the URL parser\n")
